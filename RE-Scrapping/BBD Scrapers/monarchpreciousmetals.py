...
competitorName = 'MonarchPreciousMetals'
...
logfile_path = 'Scraper logs/monarchpreciousmetals.log'
website_code = 'MP'
...
def parse_product_page(driver_source, url, CP_Id, CompPriceScraperLogId,BBDProductId):
    ...
    priceTable = data[5][col]
    priceTier = data[7][col]
    ...
    try:
        ...
        if parsed_page.xpath('//h1[text()="This Page no longer exists."]'):  # Check for product page validity
        ...
        # Metal price
        goldspotprice = parsed_page.xpath(goldSpotPrice)[0].replace("GOLD ASK:", '').strip()
        silverspotprice = parsed_page.xpath(silverSpotPrice)[0].replace("SILVER ASK:", '').strip()
        ...
        presale = 0
        availability = 1 if parsed_page.xpath(IsAvailable)[0].replace('In Stock:', '').strip() != '0' else 0
        outofstock = 1 if parsed_page.xpath(IsAvailable)[0].replace('In Stock:', '').strip() == '0' else 0
        discontinued = 0
        ...
        #     value = ''.join(detail.xpath(productDetailsValue)).replace('\xa0',' ').strip()
        #     if 'Content:' in label:
        #         metal_content = value
