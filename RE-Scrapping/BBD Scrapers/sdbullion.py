...
logfile_path = 'Scraper logs/sdbullion.log'
website_code = 'SD'
...
def parse_product_page(driver_source, url, CP_Id, CompPriceScraperLogId,BBDProductId):
    ...
    try:
        if '404 Page Not Found' in driver_source:  # Check for product page validity
        ...
        # Metal price
        # switch to iframe metal price
        driver.switch_to.frame(driver.find_element(By.XPATH,"(//div[contains(@class, 'sdb-header-charts')]//iframe)[1]"))
        ...
        parsed_page = html.fromstring(driver.page_source) #re parse after switching
        ...
        if len(parsed_page.xpath(priceTable))>0:
            base_qty_rows = parsed_page.xpath(priceTable)[0].xpath('.//tbody/tr')
        ...
        if not from_result_page:  # Collect all data
            ...
            print('BBDProductId:',BBDProductId)
...
def select_max_results_option():
    sixty_results_link = driver.find_element_by_xpath('//select[@id="limiter"]/option[@value="25"]')
