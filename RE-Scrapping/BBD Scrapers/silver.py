...
logfile_path = 'Scraper logs/silver.log'
website_code = 'Silver'
...
def parse_product_page(driver_source, url, CP_Id, CompPriceScraperLogId,BBDProductId):
    ...
    try:
        ...
        if len(base_qty_rows) >= 1:
            pricetier1 = ''.join(base_qty_rows[0].xpath('.//td[2]//text()')).strip() if base_qty_rows[0].xpath('.//td[2]//text()') else None
            if len(base_qty_rows) > 1:
                priceindex = len(base_qty_rows) -1         
                last_price_option = ''.join(base_qty_rows[priceindex].xpath('.//td[2]//text()')).strip() if base_qty_rows[priceindex].xpath('.//td[2]//text()') else None
            else:
        ...
        #     if ':' in ''.join(detail.xpath(productDetailsValue)):
        #         raw_detal = ''.join(detail.xpath(productDetailsValue)).split(':')
        #         label = raw_detal[0].strip()
        #         value = raw_detal[1].strip()

        #         if 'Content' in label or 'Weight' in label:
        #             metal_content = value
