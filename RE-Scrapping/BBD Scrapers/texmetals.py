...
logfile_path = 'Scraper logs/texmetals.log'
website_code = 'TM'
competitorId = 0
scraperLogId = 0
...
def parse_product_page(driver_source, url, CP_Id, CompPriceScraperLogId,BBDProductId):
    ...
    priceTable = '//div[@class="box-tocart"]//div[@class="price_details js-price-details"]/div[@class="price_details_prices"]'
    ...
    try:
        ...
        # Metal price
        goldspotprice = " ".join("".join(source.xpath(goldSpotPrice)).split())
        silverspotprice = " ".join("".join(source.xpath(silverSpotPrice)).split())
        platinumspotprice = " ".join("".join(source.xpath(platinumSpotPrice)).split())
        palladiumspotprice = str(0.0)
        ...
        product = source
        product_name = product.xpath(productName)[0].strip()
        metal_type = metal_utils.get_metal_type(driver, product_name,competitorName)

        presale = 0
        availability = 1 if product.xpath(IsAvailable) else 0
