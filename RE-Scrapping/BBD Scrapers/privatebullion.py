...
logfile_path = 'Scraper logs/privatebullion.log'
website_code = 'PB'
...
def parse_product_page(driver_source, url, CP_Id, CompPriceScraperLogId,BBDProductId):
    ...
    # priceTable = '//table[@class="nfs_catalog_plugin_table"]'

    priceTier = data[9][col]
    productDetailsTable = data[10][col]
    productDetailsLabel = data[11][col]
    productDetailsValue = data[12][col]
    ...
    try:
        ...
        base_qty_rows = parsed_page.xpath(priceTable+'/tbody/tr[position()>1]')
        ...
        presale = 0
        IsAvailable = './/form[@class="cart"]/button[@name="add-to-cart"]/text()'
        Isoutofstock = './/*[@class="content"]/div[2]/div[2]/p[2]/text()'
        availability = 1 if ' '.join(' '.join(parsed_page.xpath(IsAvailable)).split()) == 'In Stock' or parsed_page.xpath('.//button[@name="add-to-cart"]') else 0
        outofstock = 1 if  ' '.join(' '.join(parsed_page.xpath(Isoutofstock)).split())  == 'Out of stock' or ' '.join(' '.join(parsed_page.xpath(IsAvailable)).split()) == 'Not Available' else 0
        discontinued = 0
        # Other product details
        metal_content = None
        dataError = None

        # product_quick_specs_rows = parsed_page.xpath(productDetailsTable+'/tbody/tr')
        # for quick_detail in product_quick_specs_rows:
        #     label = ' '.join(' '.join(quick_detail.xpath(productDetailsLabel)).split())
        #     value = ' '.join(' '.join(quick_detail.xpath(productDetailsValue)).split())
        #     if value != '':
        #             if label == 'Weight':
        #                 metal_content = value
        #             else:
...
if __name__ == '__main__':
    try:
        ...
        if update_joined_only:
            ...
            # cursor.execute("""SELECT * FROM CPXpaths where CPId=?""",competitorId)
            # data = cursor.fetchall()
            # col = 3
            ...
            print('spots: ',goldspotprice,silverspotprice,platinumspotprice,palladiumspotprice )
