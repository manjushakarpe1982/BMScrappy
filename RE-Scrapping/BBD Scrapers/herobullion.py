...
logfile_path = 'Scraper logs/herobullion.log'
website_code = 'HB'
goldspotprice = None
...
def parse_product_page(driver_source, url, CP_Id, CompPriceScraperLogId,BBDProductId):
    ...
    try:
        ...
        driver.switch_to.frame(driver.find_element(By.XPATH,"(//div[contains(@class,'top-panel')]//iframe)[1]"))
        parsed_page = html.fromstring(driver.page_source)
        goldspotprice = driver.find_element(By.XPATH,'//ul/li/table[1]/tbody/tr/td[2]/span[3]').text #' '.join(parsed_page.xpath(goldSpotPrice))
        silverspotprice = driver.find_element(By.XPATH,'//ul/li/table[2]/tbody/tr/td[2]/span[3]').text#' '.join(parsed_page.xpath(silverSpotPrice))
        platinumspotprice = driver.find_element(By.XPATH,'//ul/li/table[3]/tbody/tr/td[2]/span[3]').text#' '.join(parsed_page.xpath(platinumSpotPrice))
        palladiumspotprice = driver.find_element(By.XPATH,'//ul/li/table[4]/tbody/tr/td[2]/span[3]').text#' '.join(parsed_page.xpath(palladiumSpotPrice))
        ...
        # print('=========1')
        # productName = '//h1[@class="product_title entry-title"]//text()'
        ...
        # priceTable ='//*[@id="nfs_catalog_plugin_wrapper"]'
        base_qty_rows = parsed_page.xpath(priceTable + '/table/tbody/tr')
        print('base_qty_rows:',len(base_qty_rows))
        
        # print()
        if len(base_qty_rows) > 2:
            base_qty_row = parsed_page.xpath(priceTable + '/table/tbody')
            priceTier = './tr[3]/td[2]/span/bdi//text()'
            pricetier1 = ''.join(base_qty_row[0].xpath(priceTier)).strip() if base_qty_row[0].xpath(priceTier) else None
            print('pricetier1:',pricetier1)
            print('base_qty_rows:',len(base_qty_rows))
            if len(base_qty_rows) > 1:
                base_qty_row = parsed_page.xpath(priceTable + '//table/tbody')
                print('in if')
                priceTier = './tr[6]//td[2]//span/bdi/text()'
                priceindex = len(base_qty_rows) -1      
                # print('base_qty_rows[5].xpath(priceTier)',base_qty_rows[5].xpath(priceTier))   
                last_price_option = ''.join(base_qty_row[0].xpath(priceTier)).strip() if base_qty_row[0].xpath(priceTier) else pricetier1
            else:
        ...
        print('pricetier2: ', pricetier1)
        print('last_price_option: ', last_price_option)
        print('pricetier2: ', type(pricetier1))
        print('last_price_option: ', type(last_price_option))
        print('ask1:',ask1)

        presale = 0
        # IsAvailable = './/*[@class="price-table-and-add-to-cart"]/form/button'
        # Isoutofstock = './/*[@class="content"]/div[2]/div[2]/p[2]/text()'
        availability = 1 if parsed_page.xpath(IsAvailable) else 0
        outofstock = 1 if parsed_page.xpath(IsOutofStock) else 0
        backorder = 1 if parsed_page.xpath(IsBackorder) else 0
        discontinued = 0

        if presale == 1 or backorder == 1:
            availability = 1
        ...
        if metal_content is None and BBDProductId != 0.0:
            metal_content = metal_utils.get_metal_weight(BBDProductId)

        print('metal_content: ', metal_content)


        premium, adjustedPremium, metal_weight_oz = metal_utils.compute_premium(
            str(last_price_option),
            str(metal_content),
            str(metal_type),
            str(goldspotprice),
            str(silverspotprice),
            str(platinumspotprice),
            str(palladiumspotprice),
            str(BBDProductId)
        )
        print('premium: ', premium)
        print('adjustedPremium: ', adjustedPremium)
        print('metal_weight_oz: ', metal_weight_oz)
...
if __name__ == '__main__':
    try:
        ...
        if update_joined_only:
            ...
            # col = 3
            # goldSpotPrice = Xpaths[5][col]
            # silverSpotPrice = Xpaths[6][col]
            # platinumSpotPrice = Xpaths[7][col]
            # palladiumSpotPrice = Xpaths[8][col]
