...
logfile_path = 'Scraper logs/dbscoins.log'
website_code = 'DB'
...
def parse_product_page(driver_source, url, CP_Id, CompPriceScraperLogId,BBDProductId):
    ...
    try:
        ...
        if '404 Not Found' in driver_source or '404 Error' in driver_source:    # Check for product page validity
        ...
        # Metal price
        try:
            goldspotprice = parsed_page.xpath(goldSpotPrice)[0]
            silverspotprice = parsed_page.xpath(silverSpotPrice)[0]
            platinumspotprice = parsed_page.xpath(platinumSpotPrice)[0]
            palladiumspotprice = parsed_page.xpath(palladiumSpotPrice)[0]
        ...
        if len(base_qty_rows) >= 1:
            ...
            if len(base_qty_rows) > 1:
                ...
                last_price_option = base_qty_rows[priceindex].xpath(priceTier).strip() if base_qty_rows[priceindex].xpath(priceTier) else None
        ...
        specifications = []
        labels = parsed_page.xpath(productDetailsLabel)
        values = parsed_page.xpath(productDetailsValue)
        count = 0
        # for value in values:
        #     data = ' '.join(''.join(value.xpath('./text()|.//strong/text()')).split())
        #     specifications.append(data)
        # for header in labels:
        #     header = ' '.join(''.join(header.xpath('.//text()')).split())
        #     if header == 'Weight' or header == 'Metal Content (Troy oz)':
        #         metal_content = specifications[count]
        #         if 'pound' in metal_content:
        #             metal_content = None
