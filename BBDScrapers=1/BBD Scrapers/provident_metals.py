...
competitorName = 'Provident Metals'
...
logfile_path = 'Scraper logs/provident_metals.log'
website_code = 'PM'
...
def parse_product_page(driver_source, url, CP_Id, CompPriceScraperLogId,BBDProductId):
    ...
    IsInStock = data[3][col]
    IsOutofStock = data[4][col]
    IsDiscontinued = data[5][col]
    goldSpotPrice = data[6][col]
    silverSpotPrice = data[7][col]
    platinumSpotPrice = data[8][col]
    palladiumSpotPrice = data[9][col]
    priceTable = data[10][col]
    priceTier = data[12][col]
    productDetailsTable = data[15][col]
    productDetailsLabel = data[16][col]
    productDetailsValue = data[17][col]
    ...
    try:
        ...
        source = html.fromstring(driver_source)
        # if from_result_page:
        #     source = row_result_html
        ...
        # Metal price
        goldspotprice = ' '.join(source.xpath(goldSpotPrice))
        silverspotprice = ' '.join(source.xpath(silverSpotPrice))
        platinumspotprice = ' '.join(source.xpath(platinumSpotPrice))
        palladiumspotprice = ' '.join(source.xpath(palladiumSpotPrice))

        base_qty_rows = source.xpath(priceTable)
        is_on_sale_price = source.xpath('.//table[contains(@class, "pricing-table")]//tbody/tr[contains(@class, "regular") or contains(@class, "sale")]')
        if is_on_sale_price:
            base_qty_rows = source.xpath('.//table[contains(@class, "pricing-table")]//tbody/tr[contains(@class, "sale")]')
            priceTier = './/td[3]//text()'
        ...
        product = source
        product_name = ' '.join(' '.join(product.xpath(productName)).split())

        metal_type = metal_utils.get_metal_type(driver, product_name,competitorName)

        presale = 1 if product.xpath('//div[@class= "in-stock"]/span//text()') else 0
        availability = 1 if product.xpath(IsInStock) else 0
        outofstock = 1 if product.xpath(IsOutofStock) else 0
        discontinued = 1 if product.xpath(IsDiscontinued) else 0
        ...
        # product_details_table_rows = product.xpath(productDetailsTable+'//tbody/tr')
...
def close_modal():
    ...
    # Close modal
    try:
        shh_modal_close = driver.find_element_by_xpath('//div//a[@class="sg-popup-close"]')
...
def save_main_metals_links():
    ...
    # Gold
    dict_main_metals_urls['gold'] = source.xpath(
        '//ul[@id="nav"]/li[@data-toggle="dropdown" and ./a[@ga-event-label="Gold"]]//li[@class="column"]//ul[@class="level3"]//li[contains(@class, "level4") and not(.//text()="See All")]/a/@href')
    # Copper
    dict_main_metals_urls['copper'] = source.xpath(
        '//ul[@id="nav"]/li[@data-toggle="dropdown" and ./a[@ga-event-label="Copper"]]//li[@class="column"]//ul[@class="level3"]//li[contains(@class, "level4") and not(.//text()="See All")]/a/@href')
