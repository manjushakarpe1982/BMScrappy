...
logfile_path = 'Scraper logs/usgoldbureau.log'
website_code = 'UG'
...
if __name__ == '__main__':
    try:
        ...
        if update_joined_only:
            ...
            # close_button = driver.find_element(By.XPATH,"//button[contains(@class, 'ub-emb-close')]")
            # if close_button:
            #     close_button.click()
            #     sleep(5)
            ...
            goldspotprice = " ".join(" ".join(main_page.xpath(goldSpotPrice)).split()).replace('Gold', '').strip()
            silverspotprice = " ".join(" ".join(main_page.xpath(silverSpotPrice)).split()).replace('Silver', '').strip()
            platinumspotprice = " ".join(" ".join(main_page.xpath(platinumSpotPrice)).split()).replace('Platinum', '').strip()
            palladiumspotprice = " ".join(" ".join(main_page.xpath(palladiumSpotPrice)).split()).replace('Palladium', '').strip()
