...
competitorName = 'Scottsdale Mint'
...
logfile_path = 'Scraper logs/scottsdalemint.log'
website_code = 'SC'
...
def parse_product_page(driver_source, url, CP_Id, CompPriceScraperLogId,BBDProductId):
    ...
    try:
        ...
        product_name = ' '.join(parsed_page.xpath(productName)[0:2]).strip()
        ...
        product_quick_specs_rows = parsed_page.xpath(productDetails)
        # if 'Specifications:' in product_quick_specs_rows:
        #     for quick_detail in product_quick_specs_rows:
        #         if '|' in quick_detail:
        #                 if 'troy' in quick_detail.split('|')[0].replace('–\xa0','').strip().lower() or 'grams' in quick_detail.split('|')[0].replace('–\xa0','').strip().lower() or 'gram' in quick_detail.split('|')[0].replace('–\xa0','').strip().lower() or 'kilo' in quick_detail.split('|')[0].replace('–\xa0','').strip().lower():
        #                     metal_content = quick_detail.split('|')[0].replace('–\xa0','').strip()
        ...
        if availability == 1:
            ...
            cursor.execute("UPDATE ProdPriceScraperLog set SuccessfulScrapedCompetitorCnt=?, EndTime= ? where SessionKey=?",
